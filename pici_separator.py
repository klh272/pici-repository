...
parser.add_argument("--input", type=pathlib.Path, default='ALL_PICIs.fasta')
...
if bool(g_neg) == True:
  ...
  with open(args.output + '/G_neg_PICIs.fasta', 'w') as handle:
...
if bool(sapi) == True:
  ...
  with open(args.output + '/SaPIs.fasta', 'w') as handle:
...
if bool(g_pos) == True:
  ...
  with open(args.output + '/Phage_Satellites.fasta', 'w') as handle:
