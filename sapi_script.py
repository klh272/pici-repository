...
Original file is located at
    https://colab.research.google.com/drive/1K0Ula6LYnlC8hBHNcQF0ZgGvY_IL3yyn
"""
...
for i in range(len(condensed_df)):
  if condensed_df.iloc[i,1].startswith('int'): # find integrases in df
    if condensed_df.iloc[i,2] >= 0.9: # check it integrase identity is above 90%
      ...
      #set lower and upper bounds for sis to be found (to the left and right of int)
      sis_low_lim_forward = int_prot_num + 5
      sis_high_lim_forward = int_prot_num + 12
      sis_low_lim_backward = int_prot_num - 5
      sis_high_lim_backward = int_prot_num - 12
      
      # search for co-localized sis
      for k in range(i-12,i+12):
        ...
        # otherwise get the gene number for sis  
          ...
          # check to see if it is within 5-12 genes away from the integrase  
